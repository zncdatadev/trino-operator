...
func (r *Role) ReconcileRole(ctx context.Context) (ctrl.Result, error) {
	roleCfg := r.Instance.Spec.Coordinator
	// role pdb
...
func (m *RoleGroup) RegisterResource() {
	...
	log := NewCoordinatorLogging(m.Scheme, m.Instance, m.Client, m.GroupName, lables, mergedCfg, logDataBuilder, common.Coordinator)
	dep := NewDeployment(m.Scheme, m.Instance, m.Client, m.GroupName, lables, mergedCfg, mergedCfg.Replicas)
	ing := NewIngress(m.Scheme, m.Instance, m.Client, m.GroupName, lables, mergedCfg)
	svc := NewService(m.Scheme, m.Instance, m.Client, m.GroupName, lables, mergedCfg)
	m.Reconcilers = []common.ResourceReconciler{pdb, cm, log, dep, ing, svc}
}

func (m *RoleGroup) MergeGroupConfigSpec() any {
	originMasterCfg := m.Instance.Spec.Coordinator.RoleGroups[m.GroupName]
	instance := m.Instance
	...
	return mergeConfig(instance.Spec.Coordinator, originMasterCfg)
}

func (m *RoleGroup) MergeLabels(mergedCfg any) map[string]string {
	mergedMasterCfg := mergedCfg.(*trinov1alpha1.CoordinatorSpec)
...
func mergeConfig(masterRole *trinov1alpha1.CoordinatorSpec,
